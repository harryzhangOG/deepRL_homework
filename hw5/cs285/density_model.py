...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            # 1. Compute deltas
            deltas = np.tile(states, (B,1,1)).swapaxes(0,1) - np.tile(self.means, (b,1,1))
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        self.elbo = tf.reduce_mean(self.log_likelihood - self.kl * self.kl_weight)
    ...
    def make_prior(self, z_size):
        ...
        prior_logstd = np.zeros(int(z_size), dtype=np.float32)
    ...
    def get_likelihood(self, state1, state2):
        ...
        discrim_targ = np.ones((state1.shape[0], 1))
        likelihood = self.sess.run([self.likelihood], feed_dict={self.state1: state1, self.state2: state2, self.discrim_target: discrim_targ})[0]
        return likelihood

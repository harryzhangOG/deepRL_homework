...
class FFModel(BaseModel):
    ...
    def define_forward_pass(self):
        ...
        self.next_obs_pred = self.delta_pred_unnormalized + obs_unnormalized # TODO(Q1) Predict next observation using current observation and delta prediction (not that next_obs here is unnormalized)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        if len(obs.shape)>1:
            ...
            actions = acs [None]
        feed = {self.obs_pl: observations, self.acs_pl: actions}
        new = {self.obs_mean_pl: data_statistics['obs_mean'],
        ...
        feed.update(new)
        return self.sess.run([self.next_obs_pred], feed_dict=feed)[0] # TODO(Q1) Run model prediction on the given batch of data

    def update(self, observations, actions, next_observations, data_statistics):
        feed = {self.obs_pl: observations, self.acs_pl: actions, self.delta_labels: next_observations - observations}
        new = {self.obs_mean_pl: data_statistics['obs_mean'],
               self.obs_std_pl: data_statistics['obs_std'],
               self.acs_mean_pl: data_statistics['acs_mean'],
               self.acs_std_pl: data_statistics['acs_std'],
        ...
        feed.update(new)
        # train the model
        _, loss = self.sess.run([self.train_op, self.loss], feed_dict=feed) # TODO(Q1) Run the defined train_op here, and also return the loss being optimized (on this batch of data)
        return loss
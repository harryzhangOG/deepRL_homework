...
def calculate_mean_prediction_error(env, action_sequence, models, data_statistics):
    ...
    mpe = np.mean(np.power(true_states - pred_states, 2))# TODO(Q1)
...
def sample_trajectories(env, policy, min_timesteps_per_batch, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        temp = sample_trajectory(env, policy, max_path_length, render, render_mode)
        steps_temp = get_pathlength(temp)
        paths.append(temp)
        timesteps_this_batch += steps_temp
